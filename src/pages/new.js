...
const New = () => {
  ...
  const [createRequest, { data, loading }] = useMutation(CREATE_REQUEST)

  const onFinish = values => {
    console.log('Received values of form: ', values)

    const { street, housenumber, postcode, city } = values
      ...
      <Content>
        <Row gutter={16}>
          ...
          {data && (
            <Col xs={24} md={12}>
              <Alert
                message="Gesuch erfolgreich erstellt!"
                description="Excepteur veniam duis commodo velit aute nisi ad est ad ea voluptate voluptate adipisicing quis."
                type="success"
                showIcon
              />
            </Col>
